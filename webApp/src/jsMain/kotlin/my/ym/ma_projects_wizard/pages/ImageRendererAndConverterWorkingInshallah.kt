...
fun ImageRendererAndConverterPage() {
	Box(
		modifier = Modifier
			...
			// Define the row behavior: Row 1 (auto), Row 2 (auto), Row 3 (minmax(0, 1fr))
			.gridTemplateRows {
				size(auto)
				minmax(0.px, 1.fr)
				size(auto)
			...
			// Defines the fallback for any dynamically added rows (optional safety net)
			//.gridAutoRows("auto")
	) {
		...
		Box(
			...
			InputContent(
				modifier = Modifier.fillMaxHeight().fillMaxWidth().gridColumn(1),
				contentHolderState = contentHolderState,
			)
			
			Div(attrs = Modifier.width(16.px).gridColumn(2).toAttrs())
			
			OutputContent(
				modifier = Modifier.fillMaxHeight().fillMaxWidth().gridColumn(3),
				outputValueState = outputValueState,
			)
		}
		
		Div(attrs = Modifier.height(24.px).gridRow(3).toAttrs())
	}
}
...
private fun InputContent(
	...
	Box(
		modifier = modifier.gridTemplateRows {
			size(auto)
			size(auto)
		},
		...
		FilePickerButton(
			modifier = Modifier.fillMaxWidth().gridRow(1),
			onPickFile = {
...
private fun OutputContent(
	...
	Box(
		...
		Box(
			...
			if (svgText == null) {
				SpanText(
					modifier = Modifier.overflowWrapAnywhere().gridRow(1).overflow(Overflow.Clip),
